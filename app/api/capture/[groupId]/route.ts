...
// Função para disparar automação de email com delay
...
export async function POST(
  ...
  try {
    ...
    const { name, email, phone, source, customFields, automationId, automationDelay } = body
    ...
    if (automationId) {
      const delayMs = (automationDelay || 0) * 1000 // Converter segundos para ms
      triggerAutomation(automationId, { name, email }, delayMs)
    }
