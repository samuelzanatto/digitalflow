...
import { IconMailForward, IconRefresh, IconUserOff, IconUserCheck } from '@tabler/icons-react'
...
export function UserManagementPanel({ initialUsers }: { initialUsers: AdminUser[] }) {
  ...
  const handleDisable = async (userId: string) => {
    ...
    const result = await disableUserAction({ userId, requesterId: currentUser?.id })
    handleActionResult(result)
  ...
  const handleEnable = async (userId: string) => {
    setRowPendingId(userId)
    const result = await enableUserAction({ userId, requesterId: currentUser?.id })
    handleActionResult(result)
    setRowPendingId(null)
  }
  ...
  const handleResetPassword = async (email: string) => {
    ...
    const result = await resetUserPasswordAction({ email, requesterId: currentUser?.id })
    handleActionResult(result)
    setRowPendingId(null)
  }
        ...
        <table className="min-w-full divide-y divide-white/5 text-sm text-white">
          ...
          <tbody className="divide-y divide-white/5">
            {users.map((user) => {
              ...
              const isUserAdmin = isRootAdmin || user.user_metadata?.role === 'admin'
              const rowOpacity = isBanned ? 'opacity-50' : ''
              return (
                <tr key={user.id} className="hover:bg-white/5">
                  <td className={`px-4 py-3 ${rowOpacity}`}>
                  ...
                  <td className={`px-4 py-3 ${rowOpacity}`}>
                    ...
                    {!isRootAdmin && isUserAdmin && (
                      <Badge variant="secondary" className="mt-1 bg-blue-500/10 text-blue-200">
                        Administrador
                      </Badge>
                    )}
                  </td>
                  <td className={`px-4 py-3 text-white/80 ${rowOpacity}`}>{user.email}</td>
                  <td className={`px-4 py-3 text-white/60 ${rowOpacity}`}>{formatDate(user.last_sign_in_at)}</td>
                  <td className="px-4 py-3">
                    {isBanned ? (
                      <Badge variant="secondary" className="bg-red-500/10 text-red-200">
                        desativado
                      </Badge>
                    ...
                    {isAdmin && (
                      <div className="flex justify-end gap-2">
                        <Button
                          ...
                          size="sm"
                          className="gap-2"
                          disabled={!user.email || isBanned || rowPendingId === user.email}
                        ...
                        {isBanned ? (
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            className="gap-2 border-emerald-500/50 text-emerald-300 hover:bg-emerald-500/10 w-[106px]"
                            disabled={isRootAdmin || rowPendingId === user.id}
                            onClick={() => void handleEnable(user.id)}
                          >
                            <IconUserCheck className="size-4" />
                            {rowPendingId === user.id ? 'Ativando...' : 'Reativar'}
                          </Button>
                        ) : (
                          <Button
                            type="button"
                            variant="destructive"
                            size="sm"
                            className="gap-2 w-[106px]"
                            disabled={isRootAdmin || rowPendingId === user.id}
                            onClick={() => void handleDisable(user.id)}
                          >
                            <IconUserOff className="size-4" />
                            {rowPendingId === user.id ? 'Desativando...' : 'Desativar'}
                          </Button>
                        )}
                      </div>
                    )}
